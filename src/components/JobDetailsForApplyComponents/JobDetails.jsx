  ...
  return (
    <div className="md:mt-16 mt-24 border-2 shadow-md rounded-lg border-t-0 dark:border-slate-800">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-center mx-4 items-center gap-2">
        <div className="text-lg md:text-xl text-gray-400 dark:text-white">
          Created: {job.createdAt ? new Date(job.createdAt).toLocaleDateString() : <FieldLoader />}
        </div>
        <div className="flex items-center gap-2">
          <MapPin className="hidden md:inline text-main dark:text-white" />
          <span className="md:hidden text-lg text-main dark:text-white">Location:</span>
          <span className="text-lg md:text-2xl text-gray-600 dark:text-white">{job.location}</span>
        </div>
        <div className="text-lg md:text-xl text-gray-400 dark:text-white">
      ...
      <p className="text-gray-700 my-8 dark:text-white text-base md:text-xl mx-2 break-words border border-y-slate-100 border-x-0 shadow-sm p-2 pb-5">
        <span className="block bg-gradient-to-t from-slate-50 to-slate-100 rounded-md text-center mb-4 pb-2 dark:bg-gradient-to-r dark:to-transparent w-fit dark:from-main-dark">
          Description
        </span> 
